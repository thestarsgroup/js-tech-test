# The Stars Group Tech Test

This test comes with a mock API and WebSocket server that represents a snapshot of some live football events we offer betting on.
...
Full details of the API are available [on the docker hub page](https://hub.docker.com/r/tsgtechtest/api/) and further information about terms used, example API responses and the betting domain in general are available in [the background docs](./Background.md)
...
4. Add the ability to load the outcomes for a market on demand (this could happen when the market's accordion is clicked)
...
3. Add support for displaying markets with different types (i.e. `win-draw-win` and `correct-score`) with more appropriate layouts. (See one of The Stars Group companies [live website](https://m.skybet.com) for inspiration.)
...
Full details of the API are available [on the docker hub page](https://hub.docker.com/r/tsgtechtest/api/). We would recommend running with `docker-compose up` so any logs from the API are visible in the terminal.

```bash
# running the basic image
docker run -it --rm --name tsg-tech-test-api -p 8888-8890:8888-8890 tsgtechtest/api:1.0.0
...
See the documentation [on the docker hub page](https://hub.docker.com/r/tsgtechtest/api/) for full details of the WebSocket interface.
